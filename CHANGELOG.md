...
## Unreleased
- E: Stopping a receive on the receiving device left the sender working: nothing is sent to it, and every request it made afterwards came back as a bare bad request, which reads like a hiccup — so it retried each file three times and walked the whole manifest into the same wall, nine thousand round trips for a folder of three thousand, with its row still claiming to be running; a session that is gone now says so, and the sender ends the transfer at once with "the receiver stopped the transfer" instead of a status code
