...
- N: A globe button shares the program itself, or the selected files, over a plain link: the other device scans the QR or types the shown address into a browser and downloads. Works while the dialog is open, and the screen stays on for as long as it is.
...
- I: Built on Flutter 3.47 / Dart 3.13; its new unawaited_return_in_try_block lint is off in net_server.dart, where the answer is written outside the catch on purpose.
...
- I: The AppImage build no longer chmods an appimagetool that is already executable, which ended the build when the tool came from a package.
...
- E: An incomplete build no longer sweeps OUT/, so the previous AppImage is not deleted when a new one was never made.
- I: The launcher icons are generated by 02-MakeIcons.sh instead of by every release build, so a build no longer rewrites tracked files.
